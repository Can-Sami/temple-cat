...
async def create_session(config: SessionConfig) -> dict[str, str]:
    ...
    helper = DailyRESTHelper(daily_api_key=daily_api_key)
    ...
    bot_token = await helper.get_token(room.url, exp=int(time.time()) + 60 * 60)
    user_token = await helper.get_token(room.url, exp=int(time.time()) + 60 * 60)
